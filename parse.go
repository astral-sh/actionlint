		...
		case "wait":
			...
			for _, name := range ret.Names {
				if name.ContainsExpression() {
					p.errorAt(name.Pos, "\"wait\" requires literal background step IDs")
				}
			}
		case "wait-all":
			// 'wait-all' takes no arguments.
			if e.val.Tag != "!!null" {
				p.error(e.val, "\"wait-all\" does not take a value")
			}
			ret.All = true
...
func (p *parser) parseStepExecCancel(entries []workflowMappingEntry) *ExecCancel {
	...
	for _, e := range entries {
		...
		case "cancel":
			...
			ret.Name = p.parseString(e.val, false)
			if ret.Name != nil && ret.Name.ContainsExpression() {
				p.errorAt(ret.Name.Pos, "\"cancel\" requires a literal background step ID")
			}
		case "id", "name", "continue-on-error":
			// do nothing
		default:
...
func (p *parser) parseStep(n *yaml.Node) *Step {
	...
	for _, e := range entries {
		...
		case "background":
			if e.val.Tag != "!!bool" {
				p.error(e.val, "\"background\" must be a boolean literal")
			} else {
